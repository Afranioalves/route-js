    ...
    re_entries: function (node) {
        for (var i = 0; i < node.__children__.length; i++) {
            if (node.__children__[i].__children__) {
                ...
                // console.dir(node.__children__[i]);
                node.__children__[i].remove()
            }
        }
    },
    entries: function (node, data, ch, cloned) {
        if (data instanceof DocumentFragment) {
            ...
            if (cloned) {
                for (var i = 0; data.length > 0; i++) {
                    ...
                    properties.entries(node, data[0], i, cloned)
                }
            } else {
                for (var i = 0; i < data.length; i++) {
                    cloned = false
                    if (data[i] instanceof Element) {
                        data[i] = data[i].cloneNode()
                        cloned = true
                    ...
                    properties.entries(node, data[i], i, cloned)
                }
            }
            return
    ...
    observer_callback: function (e, d) {
        ...
        if (data) {
            ...
            if (!((e = properties.store[node.__data__[0]]) && (e = e.events) && (e = e.has(node.__data__[1])))) {
                ...
                // console.log(data);
                // properties.store[node.__data__[0]].items.map(node.__data__[1],data)
                properties.entries(node, data, 0, true)
            }
        }
        // if (arguments[1]) {
        // console.log(arguments[1].__data__.toString(),node.__data__.toString());

        // }
        //         if (arguments[1]&&arguments[1].__data__.toString()===node.__data__.toString()) {
        //             return properties.console.error('out of stack')
        //         }
...
(function () {
    ...
    this.map = function (name, data) {
        if (typeof name !== "string") {
            return properties.console.error('invalid argument @map')
        }
        name = name.toLowerCase().trim()
    ...
    this.mapAll = function (data) {
        if (typeof data === 'object') {
            ...
            return properties.console.error('invalid argument @mapAll')
        }
    }
